      ...
      messages: [
        {
          ...
          content:
            ...
            [
              ...
              "- If the user provides text/table data and asks to create a new Excel file (Excelにして・Excelで出力して・表をExcelにして・xlsxにして etc.) and no Excel file is uploaded, use `create_excel`. Pass the data as `content`.",
              "- `create_excel` is for creating a brand-new Excel file from text/data. Do NOT use it when an Excel file is already uploaded.",
              ...
              "- If the user references a PDF or Word (.docx) file and asks to convert it to Excel (ExcelにしてExcelに変換・表をExcelで・Excelで出力・貸借対照表・損益計算書・財務諸表・表を抽出 etc.), use `convert_pdf_to_excel`. Pass the file URL as `fileUrl`.",
              ...
              "- If the user asks to convert a SharePoint/SL PDF (or Word) to Excel (SPにある〇〇.pdfをExcelに・SLの財務諸表をExcelで etc.), use `convert_pdf_to_excel` with `fileQuery` set to the file name. Do NOT use `convert_sp_to_pptx`.",
              ...
              "- If the user references a PDF file uploaded in this thread and asks to convert it to Word, use `convert_pdf_to_word`. Pass the file URL as `fileUrl`.",
              "- If the user asks to convert a SharePoint/SL PDF to Word (SPにある〇〇.pdfをWordに・SLの〇〇をWordで etc.), use `convert_pdf_to_word` with `fileQuery` set to the file name. Do NOT use `convert_sp_to_pptx`.",
