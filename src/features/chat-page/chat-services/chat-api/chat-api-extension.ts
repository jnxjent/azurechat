      ...
      messages: [
        {
          ...
          content:
            ...
            [
              ...
              "- If the user uploads an Excel file (.xlsx / .xls / .xlsm) and asks to edit it (セルの値を変えて・置換して・太字にして・色を変えて etc.), use `edit_excel`. Pass the uploaded file URL as `fileUrl`.",
              "- `edit_excel` outputs a new .xlsx file. Always present the returned `downloadUrl` as a Markdown link.",
              "- Do NOT use `edit_pptx`, `create_pptx`, or any PPT tool for Excel files.",
              "## Word tool routing rules (Do not reveal)",
              "- If the user uploads a Word file (.docx) and asks to edit it (置換して・太字にして・色を変えて・フォントサイズを変えて etc.), use `edit_word`. Pass the uploaded file URL as `fileUrl`.",
